...
type TabType = "audio" | "lyrics" | "sync" | "preview" | "export";
...
export default function App() {
  ...
  useEffect(() => {
    if (activeTab === "sync" && syncContainerRef.current) {
      const activeLine = syncContainerRef.current.querySelector(
        `[data-line-index="${currentIndex.lineIndex}"]`,
      );
      if (activeLine) {
  ...
  const handleResetToLine = useCallback(
    (idx: number) => {
  ...
  const tabs = [
    ...
    { id: "preview", label: "Xem lại", icon: Play },
        ...
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-xl font-bold tracking-tight text-white">
              LyricSync{" "}
              <span className="text-indigo-400 font-medium">Studio</span>
            </h1>
          </div>
      ...
      <main className="max-w-7xl mx-auto px-6 py-10 pb-40">
        <AnimatePresence mode="wait">
            ...
            >
              <div className="text-center space-y-2">
                ...
                <p className="text-slate-500">
                  Tải lên hoặc thay đổi tệp nhạc của bạn.
                </p>
              </div>
              ...
              <div className="bg-slate-900 p-8 rounded-[2.5rem] border border-slate-800 shadow-2xl">
                ...
                >
                  ...
                  <div className="flex flex-col items-center gap-6">
                    <div
                      className={`w-20 h-20 rounded-2xl flex items-center justify-center transition-transform group-hover:scale-110 ${audioFile ? "bg-emerald-500/20 text-emerald-400" : "bg-slate-800 text-slate-500"}`}
                    >
                      <Upload size={40} />
                    </div>
                    <div>
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-slate-500 uppercase tracking-widest">
                          {isReady ? "Sẵn sàng biên tập" : "Đang xử lý..."}
                        </p>
                      </div>
                    </div>
                    <button
          ...
          {activeTab === "lyrics" && (
            ...
            >
              <div className="text-center space-y-2">
                ...
                <p className="text-slate-500">
                  Dán và chỉnh sửa văn bản thô trước khi đồng bộ.
                </p>
              </div>
          ...
          {activeTab === "sync" && (
            ...
            >
              <div className="xl:col-span-8">
                <div className="bg-slate-900 rounded-[2.5rem] shadow-2xl border border-slate-800 min-h-[600px] relative">
                  {/* Sticky Header với Undo Button */}
                  <div className="sticky top-20 bg-slate-900/95 backdrop-blur-sm border-b border-slate-800 rounded-t-[2.5rem] p-6 z-10">
                    <div className="flex items-center justify-between">
                      <h3 className="text-xs font-black uppercase tracking-[0.2em] text-slate-500 flex items-center gap-2">
                        <Zap size={14} className="text-indigo-500" /> Chế độ
                        đồng bộ thời gian thực
                      </h3>
                      <div className="flex items-center gap-2">
                        <button
                          onClick={handleUndo}
                          className="flex items-center gap-2 px-4 py-2 bg-indigo-600 text-white rounded-xl text-xs font-bold hover:bg-indigo-500 transition-all shadow-lg shadow-indigo-500/20"
                        >
                          <Loader2 size={14} className="rotate-180" /> Hoàn tác
                          (Back)
                        </button>
                        <button
                          onClick={resetSync}
                  ...
                  <div ref={syncContainerRef} className="p-10 space-y-24 pb-60">
                    {syncData.length > 0 ? (
                      syncData.map((line, lIdx) => {
                        const showSection =
                          lIdx === 0 ||
                          syncData[lIdx - 1].section !== line.section;
                      ...
                      <div className="text-center py-20 space-y-4">
                        <p className="text-slate-500 italic">
                          Chưa có dữ liệu lời nhạc. Hãy quay lại tab Lời nhạc.
                        </p>
                        <button
                          onClick={() => setActiveTab("lyrics")}
              ...
              <div className="xl:col-span-4">
                ...
                <div className="sticky top-44 space-y-4">
                  <div className="bg-slate-900 rounded-4xl shadow-2xl border border-slate-800">
                    <div className="sticky top-0 bg-slate-900/95 backdrop-blur-sm border-b border-slate-800 rounded-t-4xl p-4 z-10">
                      <h3 className="text-xs font-black uppercase tracking-[0.2em] text-slate-500 flex items-center gap-2">
                        <ListMusic size={14} className="text-indigo-500" /> Tiến
                        độ đồng bộ
                      </h3>
                    </div>

                    <div className="p-4 max-h-[calc(100vh-200px)] overflow-y-auto custom-scrollbar space-y-4">
                      {/* Overall Progress */}
                      <div className="bg-gradient-to-r from-slate-800 to-slate-800/50 rounded-2xl p-4 border border-slate-700">
                        <div className="flex items-center justify-between mb-2">
                          ...
                          <span className="text-xs font-mono text-indigo-400">
                            {
                              syncData.filter((line) => line.startTime !== null)
                                .length
                            }
                            /{syncData.length} dòng
                          </span>
                        </div>
                        <div className="w-full bg-slate-950 rounded-full h-2 overflow-hidden">
                          <div
                            ...
                            style={{
                              width: `${(syncData.filter((line) => line.startTime !== null).length / syncData.length) * 100}%`,
                            }}
                          />
                        </div>
                      </div>
                      ...
                      {syncData.map((line, lIdx) => {
                        const isCurrentLine = currentIndex.lineIndex === lIdx;
                        const syncedWords = line.words.filter(
                          (w) => w.startTime !== null,
                        ).length;
                        const totalWords = line.words.length;
                        const lineProgress =
                          totalWords > 0 ? (syncedWords / totalWords) * 100 : 0;

                        return (
                          <div
                            key={lIdx}
                            className={`bg-slate-800/50 rounded-2xl p-4 border transition-all cursor-pointer hover:border-indigo-500/30 ${
                              isCurrentLine
                          ...
                          >
                            <div className="flex items-center justify-between mb-3">
                              <div className="flex items-center gap-2">
                                ...
                                {line.section && (
                                  <span className="text-[9px] px-2 py-0.5 bg-slate-700 text-slate-400 rounded-full">
                                    {line.section}
                                  </span>
                                )}
                                {isCurrentLine && (
                                  <span className="text-[9px] px-2 py-0.5 bg-indigo-500 text-white rounded-full animate-pulse">
                                    ĐANG ĐỒNG BỘ
                                  </span>
                                )}
                              </div>
                              <span className="text-[10px] font-mono text-indigo-400">
                                {syncedWords}/{totalWords} từ
                              </span>
                            </div>
          ...
          {activeTab === "preview" && (
            ...
            >
              {syncData.filter((line) => line.startTime !== null).length ===
              0 ? (
                <div className="text-center py-20 space-y-4">
                  <div className="w-20 h-20 bg-slate-800 text-slate-600 rounded-full flex items-center justify-center mx-auto">
                    <Play size={40} />
                  </div>
                  ...
                  <p className="text-slate-500">
                    Hãy hoàn thành đồng bộ lời nhạc trước khi xem lại.
                  </p>
                  <button
                ...
                <>
                  <div className="text-center space-y-2">
                    ...
                    <p className="text-slate-500">
                      Kiểm tra kết quả đồng bộ với highlight từng từ theo thời
                      gian thực.
                    </p>
                  </div>
                  ...
                  <div className="bg-slate-900 rounded-[2.5rem] p-8 shadow-2xl border border-slate-800">
                    ...
                    <div
                      ref={waveformRef}
                      className="w-full h-24 bg-slate-950 rounded-2xl mb-4"
                    />

                    <div className="flex items-center justify-between text-sm text-slate-400">
                      <span>{formatTime(currentTime)}</span>
                      <div className="flex-1 mx-4 h-2 bg-slate-950 rounded-full overflow-hidden">
                        <div
          ...
          {activeTab === "export" && (
            ...
            >
              <div className="text-center space-y-2">
                ...
                <p className="text-slate-500">
                  Tải về kết quả đồng bộ của bạn.
                </p>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="bg-slate-900 p-10 rounded-[2.5rem] border border-slate-800 shadow-2xl space-y-6 text-center">
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="text-sm text-slate-500">
                      Dữ liệu đầy đủ bao gồm word-by-word timing cho các ứng
                      dụng web.
                    </p>
                  </div>
                  <button
                    onClick={() => exportToJson(syncData)}
                ...
                <div className="bg-slate-900 p-10 rounded-[2.5rem] border border-slate-800 shadow-2xl space-y-6 text-center">
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="text-sm text-slate-500">
                      Định dạng phụ đề tiêu chuẩn cho video và trình phát nhạc.
                    </p>
                  </div>
                  <button
                    onClick={() => exportToSrt(syncData)}
                    className="w-full py-4 bg-emerald-600 text-white rounded-2xl font-bold hover:bg-emerald-500 transition-all"
                  >
                    Tải SRT
                  </button>
                </div>
      ...
      <footer className="py-12 border-t border-slate-800/50 mt-20">
        <div className="max-w-7xl mx-auto px-6 text-center">
          <p className="text-slate-500 text-sm font-medium tracking-wide">
            ©LyricSync Studio được phát triển bởi{" "}
            <span className="text-indigo-400 font-bold">An Kun Studio</span>.
            All right reserver.
          </p>
        </div>
      </footer>
